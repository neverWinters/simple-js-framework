...
module.exports = {
    createProject: async(paramsObj) => {
        try{
            ...
            let authenticationJsTemplate = projectTemplate.public.js.authentication.join('\n');
    ...
    createComponent: async(paramsObj) => {
        try{
            ...
            let jsTemplate = componentTemplate.js.languageVariants.filter((template) => template.multiLanguageSupport == projectPackageJson.customGen.multiLanguageSupport);
            let globalsJsTemplate = jsTemplate[0].globals.join('\n');
            ...
            if(projectPackageJson.customGen.multiLanguageSupport){
                componentJsTemplate = componentJsTemplate.replace(/<--COMPONENT_NAME-->/gi, componentLangObject);
            ...
            if(paramsObj.authenticationType == 'Global'){ indexTemplate = indexTemplate.replace(/<--AUTHENTICATION_ROUTE-->/gi, '../js/authentication.js'); }
                ...
                let authenticationJsTemplate = componentTemplate.js.authentication.join('\n');
                fs.appendFileSync(`./public/${paramsObj.componentName}/js/authentication.js`, authenticationJsTemplate);
            }
